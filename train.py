...
class AngleDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.augment:
            # Rotate the complete RGBA sample clockwise and update the label
            # by the same positive angle. np.rot90(k=-1) is clockwise.
            if random.random() < 0.5:
                quarter_turns = random.choice((1, 2, 3))
                array = np.rot90(array, k=-quarter_turns, axes=(0, 1)).copy()
                angle = (angle + 90 * quarter_turns) % 360
        rgb = array[..., :3]
        alpha = array[..., 3:4]
        if self.augment:
            if random.random() < 0.5:
