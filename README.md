...
默认架构是 ConeCNN（`architecture = "cone"`）：先把极坐标条带逐像素评分，沿半径聚合为 360 维方位角剖面，再经循环一维卷积匹配滤波定位视野扇形，输出每个方位角的 logits 后 argmax 解码；方位角轴全程不下采样、只做循环卷积，对输入平移精确等变。旧基线 AngleCNN 保留为配置可选值 `angle_cnn`，供对照复现。
